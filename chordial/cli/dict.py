  ...
  if d := ctx.session.query(Dictionary).filter_by(id=id).first():
    try:
      with ProgressBar() as p:
        collect = p.add_task("[cyan]Reading existing database", start=False, total=None)
        read, delete, add_ol, add_tl, add_e, commit = None, None, None, None, None, None
        ...
        def begin_commit_callback(item):
          nonlocal commit
          commit = p.add_task(f"[blue]Committing {item}", start=False, total=None)
...
def dict_list(ctx):
  ...
  rows = [
    (d.id, d.short_name, len(d.entries))
    for d in dicts
  ]
